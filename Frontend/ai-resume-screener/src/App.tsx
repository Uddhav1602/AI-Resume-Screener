    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-16" style={{ backgroundColor: "#0d1117" }}>
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-md flex items-center justify-center text-blue-400 text-sm font-bold" style={{ backgroundColor: "#1e2d3d" }}>
                ☁
              </div>
          ...
          <div
            ...
            style={{
              minHeight: "220px",
              backgroundColor: dragging ? "#1a2535" : "#131c27",
              borderColor: dragging ? "#60a5fa" : file ? "#22c55e50" : "#2a3a4e",
            }}
          >
            ...
            {file ? (
              <>
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); setFile(null); setAnalyzed(false); }}
                  className="text-gray-500 hover:text-red-400 text-xs mt-3 transition-colors duration-150"
                >
                  Remove file
                </button>
              </>
              ...
              <>
                ...
                <span className="text-xs text-gray-400 px-3 py-1 rounded-full" style={{ backgroundColor: "#1e2d3d" }}>
                  .pdf files only
                </span>
              </>
            )}
          ...
          <button
            onClick={handleUploadClick}
            className="w-full py-3 rounded-xl text-white font-medium text-sm flex items-center justify-center gap-2 transition-all duration-150 hover:opacity-90 active:scale-95"
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-md flex items-center justify-center text-purple-400 text-sm font-bold" style={{ backgroundColor: "#2a1e3d" }}>
                ✎
              </div>
          ...
          <button
            onClick={() => { setJobDescription(""); setAnalyzed(false); }}
            disabled={!jobDescription}
            className="w-full py-3 rounded-xl text-gray-400 font-medium text-sm flex items-center justify-center gap-2 transition-all duration-150 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ backgroundColor: "#1b2b3a" }}
          >
            🗑 Clear Description
          </button>
      ...
      <div className="mt-8 flex flex-col items-center gap-2">
        <button
          ...
          style={{
            background: canAnalyze && !analyzing ? "linear-gradient(135deg, #2563eb, #7c3aed)" : "#1e2d3d",
            boxShadow: canAnalyze && !analyzing ? "0 0 24px rgba(96, 165, 250, 0.25)" : "none",
          }}
        >
        ...
        {!canAnalyze && (
          <p className="text-gray-600 text-xs">
            {!file && !jobDescription ? "Upload a resume and add a job description to begin"
              : !file ? "Upload a PDF resume to continue"
              : "Add a job description to continue"}
          </p>
        )}

        {analyzed && (
          <p className="text-green-400 text-sm font-medium mt-1">✓ Analysis complete! Results are ready.</p>
        )}
      </div>
    </div>
  );
